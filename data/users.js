...
const getUsersCollection= mongoCollections.usersCollection;
...
module.exports = {
	...
	async addSessionForUser(username,sessionId) {
		const user = await this.getUserByUsername(username);
		let sessionData = user["sessions"];
		sessionData.push(sessionId);

		return updatedUser = await this.updateUser(user["_id"], {"sessions": sessionData});
	},
	
	async removeSessionForUser(username, sessionId) {
		const user = await this.getUserByUsername(username);
		let sessionData = user["sessions"];
		var sessionIndex = sessionData.indexOf(sessionId);
		if (sessionIndex !== -1) {
			sessionData = sessionData.splice(sessionIndex, 1);
			return updatedUser = await this.updateUser(user["_id"], {"sessions": sessionData});
		}
		else {
			//session not found
